  ...
  * The name of a plugin, prefixed with the parser name and a '/', for example
  ...
  def _JoinExpression(self, excludes, includes):
    ...
    if missing_parser_filters:
      ...
      raise RuntimeError(
          f'Parser filters: {",".join(missing_parser_filters):s} '
          f'defined to be excluded but no corresponding include expression')
  ...
  def _ExpandPreset(self, presets_manager, preset_name, parsers_and_plugins):
    ...
    if plugins != set(['*']):
      raise RuntimeError(
          f'{preset_name:s} cannot be used as a preset name '
          f'and plugin name at same time.')
