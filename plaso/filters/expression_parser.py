...
class EventFilterExpressionParser:
    ...
    _TOKENS = [
        ...
        Token(_STATE_INITIAL, r"[^\s\(\)]", "_PushState,_PushBack", _STATE_ATTRIBUTE),
        ...
        Token(_STATE_DATETIME, r"(\d+)", "_SetDateTimeDecimalInteger", _STATE_DATETIME),
        Token(
            _STATE_DATETIME, '"', "_PushState,_StringStart", _STATE_STRING_DOUBLE_QUOTE
        ),
        Token(
            _STATE_DATETIME, "'", "_PushState,_StringStart", _STATE_STRING_SINGLE_QUOTE
        ),
        # Path definition
        Token(_STATE_PATH, r"\)", "_PopState,_AddArgumentPath", None),
        Token(_STATE_PATH, '"', "_PushState,_StringStart", _STATE_STRING_DOUBLE_QUOTE),
        Token(_STATE_PATH, "'", "_PushState,_StringStart", _STATE_STRING_SINGLE_QUOTE),
        # Basic expression
        Token(_STATE_ATTRIBUTE, r"[\w._0-9]+", "_SetAttribute", _STATE_OPERATOR),
        Token(_STATE_OPERATOR, "not ", "_NegateExpression", None),
        Token(
            _STATE_OPERATOR, r"(\w+|[<>!=]=?)", "_SetOperator", _STATE_NEGATION_OPERATOR
        ),
        Token(_STATE_NEGATION_OPERATOR, "not", "_NegateExpression", _STATE_ARGUMENT),
        Token(_STATE_NEGATION_OPERATOR, r"\s+", None, None),
        Token(_STATE_NEGATION_OPERATOR, r"([^not])", "_PushBack", _STATE_ARGUMENT),
        Token(
            _STATE_ARGUMENT, r"(\d+\.\d+)", "_AddArgumentFloatingPoint", _STATE_ARGUMENT
        ),
        Token(
            _STATE_ARGUMENT,
            r"(0x\d+)",
            "_AddArgumentHexadecimalInteger",
            _STATE_ARGUMENT,
        ),
        Token(_STATE_ARGUMENT, r"(\d+)", "_AddArgumentDecimalInteger", _STATE_ARGUMENT),
        Token(
            _STATE_ARGUMENT, '"', "_PushState,_StringStart", _STATE_STRING_DOUBLE_QUOTE
        ),
        Token(
            _STATE_ARGUMENT, "'", "_PushState,_StringStart", _STATE_STRING_SINGLE_QUOTE
        ),
    ...
    def _AddArgumentDateTime(self, **unused_kwargs):
        ...
        if isinstance(self._datetime_value, int):
            ...
            try:
                ...
                raise errors.ParseError(
                    f"unsupported ISO 8601 string: {self._datetime_value:s}."
                )

        else:
            raise errors.ParseError(
                f"unsupported datetime value: {self._datetime_value!s}."
            )
    ...
    def _AddArgumentHexadecimalInteger(self, string="", **unused_kwargs):
        ...
        try:
            ...
            raise errors.ParseError(f"{string:s} is not a valid base16 integer.")
        return self._AddArgument(int_value)
    ...
    def _CombineBinaryExpressions(self, operator):
        ...
        while item_index < number_of_items:
            ...
            if (
                isinstance(item, expressions.BinaryExpression)
                ...
                if isinstance(previous_item, expressions.Expression) and isinstance(
                    next_item, expressions.Expression
                ):
    ...
    def _CombineParenthesis(self):
        ...
        while item_index < number_of_items:
            ...
            if (
                previous_item == "("
                and next_item == ")"
                and isinstance(item, expressions.Expression)
            ):
                self._stack.pop(item_index + 1)
                self._stack.pop(item_index - 1)
    ...
    def _GetNextToken(self):
        ...
        number_of_bytes = len(self._processed_buffer)
        raise errors.ParseError(
            f"No token match for parser state: {self._state:s} at position "
            f"{number_of_bytes:d}: {self._processed_buffer:s} <---> "
            f"{self._buffer:s}"
        )
    ...
    def _PopState(self, **unused_kwargs):
        ...
        try:
            ...
            number_of_bytes = len(self._processed_buffer)
            raise errors.ParseError(
                f"Tried to pop state from an empty stack - possible recursion error "
                f"at position {number_of_bytes:d}: {self._processed_buffer:s} <---> "
                f"{self._buffer:s}"
            )
    ...
    def _PushState(self, **unused_kwargs):
        ...
        Returns:
          str: next state, which is None.
        """
        logger.debug(f"Storing state {self._state:s}")
    ...
    def _Reduce(self):
        ...
        if self._state not in (self._STATE_BINARY_OPERATOR, self._STATE_INITIAL):
            number_of_bytes = len(self._processed_buffer)
            raise errors.ParseError(
                f"Unsupported initial state: {self._state:s} - premature end of "
                f"expression at position {number_of_bytes:d}: "
                f"{self._processed_buffer:s} <---> {self._buffer:s}"
            )
        ...
        if number_of_items != 1:
            number_of_bytes = len(self._processed_buffer)
            raise errors.ParseError(
                f"Unsupported event filter expression at position "
                f"{number_of_bytes:d}: {self._processed_buffer:s} <---> "
                f"{self._buffer:s}"
            )
    ...
    def _SetAttribute(self, string="", **unused_kwargs):
        ...
        """
        logger.debug(f"Storing attribute {string:s}")
