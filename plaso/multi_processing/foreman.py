  ...
  * the memory consumption of the worker.
  ...
  def _CheckStatus(self, process_label):
    ...
    if status_indicator not in [
      ...
      # list of labels.
      process = self._processes_per_pid[process_label.pid]
      self.StopProcessMonitoring(process)
  ...
  def _UpdateProcessingStatus(self, process_label, process_status):
    ...
    if process_type == u'collector':
      self.processing_status.number_of_path_specs = process_status.get(
          u'number_of_path_specs', None)

      self.processing_status.collector_completed = (
          status_indicator == definitions.PROCESSING_STATUS_COMPLETED)

    elif process_type == u'storage_writer':
      self.processing_status.number_of_events = process_status.get(
          u'number_of_events', 0)
      number_of_events = self.processing_status.GetNumberOfExtractedEvents()

      if (self.processing_status.extraction_completed and
          self.processing_status.number_of_events == number_of_events):
        self.processing_status.processing_completed = True
      else:
        self.processing_status.processing_completed = False
      ...
      if is_running:
        status = process_label.process_information.status
      else:
        status = status_indicator

      self.processing_status.UpdateExtractionWorkerStatus(
          process_label.label, process_label.pid, display_name,
          number_of_events, number_of_path_specs, status, is_running)

      number_of_path_specs = (
          self.processing_status.GetNumberOfExtractedPathSpecs())

      # TODO: include the path specs generated by the workers.
      if (self.processing_status.collector_completed and
          self.processing_status.number_of_path_specs == number_of_path_specs):
        self.processing_status.extraction_completed = True
      else:
        self.processing_status.extraction_completed = False
  ...
  def CheckStatus(self):
    ...
    if self.processing_status.processing_completed:
      logging.debug(u'Processing completed.')

    return self.processing_status.processing_completed
  ...
  def StopProcessMonitoring(self, process):
    ...
    Raises:
      KeyError: if the process PID is not in the monitoring list.
      ValueError: if the process object is missing.
    """
    if not process:
      raise ValueError(u'Missing process object.')

    if process.pid not in self._monitored_process_labels:
      raise KeyError(u'Process (PID: {0:d}) not in monitoring list.'.format(
          process.pid))

    process_label = self._monitored_process_labels[process.pid]
    del self._monitored_process_labels[process.pid]

    # Remove the RPC client.
    rpc_client = self._rpc_clients_per_pid.get(process.pid, None)
    if rpc_client:
      rpc_client.Close()
      del self._rpc_clients_per_pid[process.pid]

    logging.info((
        u'Process: {0:s} (PID: {1:d}) has been removed from the monitored '
        u'list.').format(process_label.label, process.pid))
