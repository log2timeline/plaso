...
class AzureApplicationGatewayAccessLogJSONLPlugin(interface.JSONLPlugin):
    ...
    def _ParseRecord(self, parser_mediator, json_dict):
        ...
        event_data.request_uri = self._GetJSONValue(properties_json_dict, "requestUri")
        event_data.sent_bytes = self._GetJSONValue(properties_json_dict, "sentBytes")
        ...
        event_data.ssl_cipher = self._GetJSONValue(properties_json_dict, "sslCipher")
        ...
        event_data.ssl_enabled = self._GetJSONValue(properties_json_dict, "sslEnabled")
        event_data.ssl_protocol = self._GetJSONValue(
            properties_json_dict, "sslProtocol"
        )
        event_data.time_taken = self._GetJSONValue(properties_json_dict, "timeTaken")
        event_data.transaction_identifier = self._GetJSONValue(
            properties_json_dict, "transactionId"
        )
        event_data.user_agent = self._GetJSONValue(properties_json_dict, "userAgent")
    ...
    def CheckRequiredFormat(self, json_dict):
        ...
        if (
            None in (operation_name, properties, iso8601_string)
            or operation_name != "ApplicationGatewayAccess"
        ):
            return False
...
jsonl_parser.JSONLParser.RegisterPlugin(AzureApplicationGatewayAccessLogJSONLPlugin)
