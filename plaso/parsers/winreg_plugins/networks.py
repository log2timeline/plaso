...
class NetworksPlugin(interface.WindowsRegistryPlugin):
  ...
  def _GetNetworkInfo(self, signatures_key):
    ...
    """
    network_info = {}
    for subkey in signatures_key.GetSubkeys():
      for subkey in subkey.GetSubkeys():
        profile_guid = subkey.GetValueByName(u'ProfileGuid')
        if profile_guid:
          profile_guid = profile_guid.GetDataAsObject()
        else:
          continue

        default_gateway_mac = subkey.GetValueByName(u'DefaultGatewayMac')
        if default_gateway_mac:
          default_gateway_mac = default_gateway_mac.GetDataAsObject()
          default_gateway_mac = u':'.join(
              map(binascii.hexlify, default_gateway_mac))

        dns_suffix = subkey.GetValueByName(u'DnsSuffix')
        if dns_suffix:
          dns_suffix = dns_suffix.GetDataAsObject()
  ...
  def GetEntries(self, parser_mediator, registry_key, **kwargs):
    ...
    if profiles:
      for subkey in profiles.GetSubkeys():
        ...
        ssid = subkey.GetValueByName(u'ProfileName')
        if ssid:
          ssid = ssid.GetDataAsObject()

        description = subkey.GetValueByName(u'Description')
        if description:
          description = description.GetDataAsObject()

        connection_type = subkey.GetValueByName(u'NameType')
        if connection_type:
          connection_type = connection_type.GetDataAsObject()
          connection_type = self._CONNECTION_TYPE.get(
              connection_type, u'unknown')

        date_created = subkey.GetValueByName(u'DateCreated')
        if date_created:
          date_created = timelib.Timestamp.FromSystemtime(
              date_created.GetDataAsObject())
          event_object = windows_events.WindowsRegistryNetworkEvent(
              date_created, eventdata.EventTimestamp.CREATION_TIME,
        ...
        if date_last_connected:
          date_last_connected = timelib.Timestamp.FromSystemtime(
              date_last_connected.GetDataAsObject())
          event_object = windows_events.WindowsRegistryNetworkEvent(
              date_last_connected, eventdata.EventTimestamp.LAST_CONNECTED,
              ssid, description, connection_type, default_gateway_mac,
              dns_suffix)
          parser_mediator.ProduceEvent(event_object)
