...
class UserAssistWindowsRegistryKeyPathFilter(interface.WindowsRegistryKeyPathFilter):
...
class UserAssistPlugin(interface.WindowsRegistryPlugin, dtfabric_helper.DtFabricHelper):
    ...
    _DEFINITION_FILE = os.path.join(os.path.dirname(__file__), "userassist.yaml")
        ...
        for registry_value in count_subkey.GetValues():
            ...
            if format_version == 5:
                ...
                for segment_index, path_segment in enumerate(path_segments):
                    path = known_folders.WindowsKnownFoldersHelper.GetPath(path_segment)
                    if path:
