...
class MRUListStringRegistryKeyFilter(interface.WindowsRegistryKeyWithValuesFilter):
    """Windows Registry key with values filter."""

    _IGNORE_KEY_PATH_SUFFIXES = frozenset(["\\Explorer\\DesktopStreamMRU".upper()])
...
class BaseMRUListWindowsRegistryPlugin(
    ...
    _DEFINITION_FILE = os.path.join(os.path.dirname(__file__), "mru.yaml")
...
class MRUListShellItemListWindowsRegistryPlugin(BaseMRUListWindowsRegistryPlugin):
...
winreg_parser.WinRegistryParser.RegisterPlugins(
    [MRUListStringWindowsRegistryPlugin, MRUListShellItemListWindowsRegistryPlugin]
)
