...
Plist files are only one example of a type of object that the Plaso tool is expected to
encounter and process. There can be and are many other parsers which are designed to
process specific data types.

PlistPlugin defines the attributes necessary for registration, discovery and operation
of plugins for plist files which will be used by PlistParser.
"""
...
class PlistPlugin(plugins.BasePlugin):
    ...
    def _GetKeys(self, top_level, keys, depth=1):
        ...
        if depth == 1:
            ...
            for _, parsed_key, parsed_value in self._RecurseKey(top_level, depth=depth):
