...
class OneDriveLogFileParser(interface.FileEntryParser, dtfabric_helper.DtFabricHelper):
    ...
    _DEFINITION_FILE = os.path.join(os.path.dirname(__file__), "onedrive.yaml")
    ...
    def _ParseObfuscatedStringMap(self, obfuscated_string_map_entry):
        ...
        with dfvfs_text_file.TextFile(
            file_object, encoding="utf-16-le"
        ) as obfuscated_text_file:
    ...
    def _DecryptAES(self, aes_key, encrypted_data):
        ...
        return pycaes.crypt_cbc(
            aes_context, pycaes.crypt_modes.DECRYPT, self._AES_IV, encrypted_data
        )
    ...
    def _ProcessRawParameters(
        self, parser_mediator, raw_parameters_data, aes_key, obfuscated_string_map
    ):
    ...
    def ParseFileEntry(self, parser_mediator, file_entry):
        ...
        """
        onedrivelog_file_header_map = self._GetDataTypeMap("onedrivelog_file_header")
        ...
        while stream_offset + odl_data_block_header_map.GetSizeHint() < data_block_size:
            try:
