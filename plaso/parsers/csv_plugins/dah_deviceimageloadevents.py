...
class DefenderAHDeviceImageLoadEventsPlugin(interface.CSVPlugin):
  """Parse DeviceImageLoadEvents from CSV files."""  

  NAME = 'dah_deviceimageloadevents'
  DATA_FORMAT = 'M365 Defender DeviceImageLoadEvents table'
  ...
  def _ParseImageLoaded(self, parser_mediator, row):
    """Extracts ImageLoaded action from a CSV file.
    ...
    event_data = dah_events.DefenderAHImageLoadedEventData()
  ...
  def _ParseCsvRow(self, parser_mediator, row):
    ...
    try:
      ...
      if tmp_action == 'imageloaded':
        self._ParseImageLoaded(parser_mediator, tmp_row)
...
csv_parser.CSVFileParser.RegisterPlugin(DefenderAHDeviceImageLoadEventsPlugin)
