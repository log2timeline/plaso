...
class JpegExifParser(interface.FileObjectParser):
  ...
  def _ExtractExifMetadata(self, file_object):
    ...
    event_data.makeandmodel = ed.get('Make and Model', 'Unknown')
    event_data.manufacturer = ed.get('Manufacturer', 'Unknown')
    event_data.model = str(ed.get('Model'))
    event_data.xres = str(ed.get('X-Resolution'))
    event_data.yres = str(ed.get('Y-Resolution'))
    event_data.width = str(ed.get('Image Width'))
    event_data.height = str(ed.get('Image Height'))
    event_data.software = str(ed.get('Software'))
    event_data.bodyserialnumber = str(ed.get('Body Serial Number'))
