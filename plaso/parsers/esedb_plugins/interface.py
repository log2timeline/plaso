...
class ESEDBPlugin(plugins.BasePlugin, dtfabric_helper.DtFabricHelper):
    ...
    BINARY_DATA_COLUMN_TYPES = frozenset(
        [pyesedb.column_types.BINARY_DATA, pyesedb.column_types.LARGE_BINARY_DATA]
    )
    ...
    _DEFINITION_FILE = os.path.join(os.path.dirname(__file__), "types.yaml")
    ...
    def _GetRecordValues(
        self, parser_mediator, table_name, record_index, record, value_mappings=None
    ):
