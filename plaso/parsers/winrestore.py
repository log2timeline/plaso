...
class RestorePointLogParser(interface.FileObjectParser, dtfabric_helper.DtFabricHelper):
    """A parser for Windows Restore Point (rp.log) files."""

    NAME = "rplog"
    DATA_FORMAT = "Windows Restore Point log (rp.log) file"

    FILTERS = frozenset([interface.FileNameFileEntryFilter("rp.log")])

    _DEFINITION_FILE = os.path.join(os.path.dirname(__file__), "winrestore.yaml")
