...
class SantaTextPlugin(interface.TextPlugin):
  ...
  _DATE_AND_TIME = (
      ...
      _TWO_DIGITS + pyparsing.Suppress(':') +
      _TWO_DIGITS + pyparsing.Suppress('.') +
      _THREE_DIGITS + pyparsing.Suppress('Z')).set_results_name('date_time')
  ...
  _APPEARANCE = (pyparsing.Suppress('|appearance=') +
                 _DATE_AND_TIME.set_results_name('appearance'))

  _ARGS = (pyparsing.Suppress('|args=') +
           _SKIP_TO_SEPARATOR.set_results_name('args'))
  ...
  _DECISION = (pyparsing.Suppress('|decision=') +
               _SKIP_TO_SEPARATOR.set_results_name('decision'))

  _DMG_PATH = (pyparsing.Suppress('|dmgpath=') +
               _SKIP_TO_SEPARATOR.set_results_name('dmg_path'))

  _EXPLAIN = (pyparsing.Suppress('|explain=') +
              _SKIP_TO_SEPARATOR.set_results_name('explain'))

  _FILE_SYSTEM = (pyparsing.Suppress('|fs=') +
                  _SKIP_TO_SEPARATOR.set_results_name('fs'))

  _GID = (pyparsing.Suppress('|gid=') +
          _SKIP_TO_SEPARATOR.set_results_name('gid'))

  _GROUP = (pyparsing.Suppress('|group=') +
            _SKIP_TO_SEPARATOR.set_results_name('group'))

  _MODE = (pyparsing.Suppress('|mode=') +
           _SKIP_TO_SEPARATOR.set_results_name('mode'))

  _MODEL = (pyparsing.Suppress('|model=') +
            _SKIP_TO_SEPARATOR.set_results_name('model'))

  _MOUNT = (pyparsing.Suppress('|mount=') +
            _SKIP_TO_SEPARATOR.set_results_name('mount'))

  _NEW_PATH = (pyparsing.Suppress('|newpath=') +
               _SKIP_TO_SEPARATOR.set_results_name('newpath'))
  ...
  _REASON = (pyparsing.Suppress('|reason=') +
             _SKIP_TO_SEPARATOR.set_results_name('reason'))

  _SERIAL = (pyparsing.Suppress('|serial=') +
             _SKIP_TO_SEPARATOR.set_results_name('serial'))

  _SHA256 = (pyparsing.Suppress('|sha256=') +
             _SKIP_TO_SEPARATOR.set_results_name('sha256'))

  _UID = (pyparsing.Suppress('|uid=') +
          _SKIP_TO_SEPARATOR.set_results_name('uid'))

  _USER = (pyparsing.Suppress('|user=') +
           _SKIP_TO_SEPARATOR.set_results_name('user'))

  _VOLUME = (pyparsing.Suppress('|volume=') +
             _SKIP_TO_SEPARATOR.set_results_name('volume'))
  ...
  _FILE_OPERATION_LINE = (
      _DATE_TIME_BLOCK + _SANTAD_PREAMBLE + pyparsing.Suppress('action=') + (
          ...
          pyparsing.Literal('WRITE')).set_results_name('action') +
