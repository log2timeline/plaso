"""Text plugin for Jira access log (jira_access_log[DATE].log) files.
...
class JiraAccessEventData(events.EventData):
    """Jira access event data.
...
class JiraAccessTextPlugin(interface.TextPlugin):
    """Text plugin for Jira access log (jira_access_log[DATE].log) files."""

    NAME = "jira_access"
    DATA_FORMAT = "Jira access log (access.log) file"
    ...
    _TIME_ZONE_OFFSET = pyparsing.Word("+-", exact=1) + _TWO_DIGITS + _TWO_DIGITS
    ...
    _THREAD_NAME = pyparsing.Word(pyparsing.alphanums + "-").set_results_name(
        "thread_name"
    )
    ...
    _REMOTE_NAME = (
        _IP_ADDRESS | pyparsing.Word(pyparsing.alphanums + "-" + ".")
    ...
    # Default (pre 9.4) format log line:
    # %t %{X-AUSERNAME}o %I %h %r %s %Dms %b %{Referer}i %{User-Agent}i
    _PRE_94_FORMAT_LOG_LINE = (
        _DATE_TIME
        + _USER_NAME
        + _THREAD_NAME
        + _REMOTE_NAME
    ...
    # Post 9.4 format log line (adds X-Forwarded-For):
    # %t %{X-Forwarded-For}i %{X-AUSERNAME}o %I %h %r %s %Dms %b %{Referer}i
    # %{User-Agent}i
    _POST_94_FORMAT_LOG_LINE = (
        _DATE_TIME
    ...
    _LINE_STRUCTURES = [
        ("pre_94_format", _PRE_94_FORMAT_LOG_LINE),
        ("post_94_format", _POST_94_FORMAT_LOG_LINE),
    ]

    VERIFICATION_GRAMMAR = _PRE_94_FORMAT_LOG_LINE ^ _POST_94_FORMAT_LOG_LINE
    ...
    def _ParseRecord(self, parser_mediator, key, structure):
        ...
        event_data = JiraAccessEventData()
        ...
        event_data.http_version = self._GetValueFromStructure(structure, "http_version")
...
text_parser.TextLogParser.RegisterPlugin(JiraAccessTextPlugin)
