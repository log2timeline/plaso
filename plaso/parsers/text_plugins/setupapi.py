...
class SetupAPILogTextPlugin(interface.TextPlugin):
    ...
    _DEVICE_INSTALL_LOG_LINE = pyparsing.Literal("[Device Install Log]") + _END_OF_LINE
    ...
    # Using a regular expression here is faster. Note that pyparsing 2 does not
    # properly handle leading whitespace.
    _LOG_ENTRY_LINE = pyparsing.Regex(r"(|\. |!    |!!!  )[A-Za-z\.]{2,3}: .{0,336}\n")
    ...
    def _ParseRecord(self, parser_mediator, key, structure):
        ...
        if key == "section_header_line":
            ...
            start_time_structure = self._GetValueFromStructure(structure, "start_time")
            ...
            end_time_structure = self._GetValueFromStructure(structure, "end_time")
