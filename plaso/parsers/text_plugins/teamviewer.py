...
class TeamViewerApplicationLogTextPlugin(interface.TextPluginWithLineContinuation):
    ...
    _LOG_LINE = (
        ...
        .set_results_name("process_identifier")
        +
...
class TeamViewerConnectionsOutgoingLogTextPlugin(interface.TextPlugin):
    ...
    _TEAMVIEWER_ID = pyparsing.Word(pyparsing.nums, min=8, max=11).set_parse_action(
        lambda tokens: int(tokens[0], 10)
    )
    ...
    def _ParseTimeElements(self, time_elements_structure):
        ...
        """
        try:
            day_of_month, month, year, hours, minutes, seconds = time_elements_structure
