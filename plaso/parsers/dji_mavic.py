...
class DJIDroneLogEventData(events.EventData):
  ...
  Attributes:
    ...
    longitude (TODO): TODO.
    timestamp (TODO): TODO.
    x_velocity (TODO): TODO.
    y_velocity (TODO): TODO.
    z_velocity (TODO): TODO.
  """
...
class DJIDroneLogParser(interface.FileObjectParser):
  ...
  def CheckSum(self, data):
    ...
    v = 13970
    for i in data:
      v = (v >> 8) ^ self._CRC_TABLE[(i ^ v) & 0xFF]
    return v
  ...
  def ParseRecords(self):
    ...
    while True:
      ...
      crc = self.CheckSum(self.data[cur_pos:cur_pos + record_len - 2])

      if (crc & 0xFF != self.data[cur_pos + record_len - 2] or
          crc >> 8 != self.data[cur_pos + record_len - 1]):
        cur_pos += record_len
        cur_pos = self.FindNext(cur_pos)
        if cur_pos == -1:
          break
      ...
      ticket_number = struct.unpack(
          "<I", self.data[cur_pos + 6:cur_pos + 10])[0]
      record_type = (self.data[cur_pos + 5] << 8) + self.data[cur_pos + 4]
      record_list.append([cur_pos, record_len, ticket_number, record_type])
  ...
  def ParseGPS(self, value):
    ...
    Returns:
      list[list]: A list of parsed GPS records with values: year, month, day,
          hour, minute, second, longitude, latitude, height, x_velocity,
    ...
    for x in records:
      if x[3] == 2096:
        payload = struct.unpack("<IIiiifff", GetPayload(x[0], x[1], x[2])[:32])
        date = payload[0]
        time = payload[1]
        ...
        gps_records.append([
            ...
            height, x_offset, y_offset, z_offset])
