...
class BsmParser(interface.FileObjectParser):
    ...
    """
        ...
        if parser_mediator.platform == u'MacOSX':
            ...
            return MacBsmEvent(event_type, timestamp, extra_tokens,
                               return_value, trailer, offset)
        else:
            # Generic BSM format.
            if extra_tokens:
                trailer = extra_tokens.get(u'BSM_TOKEN_TRAILER', 'unknown')
            return BsmEvent(event_type, timestamp, extra_tokens, trailer,
                            offset)
    ...
    """
        ...
        if file_object.tell() != next_entry:
            # Unknown Structure.
            logging.warning(u'Unknown Token at "0x{0:X}", ID: {1} (0x{2:X})'.
                            format(start_position - 1, token_id, token_id))
