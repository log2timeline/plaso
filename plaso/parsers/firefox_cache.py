...
class FirefoxCacheParser(BaseFirefoxCacheParser, dtfabric_helper.DtFabricHelper):
    ...
    _DEFINITION_FILE = os.path.join(os.path.dirname(__file__), "firefox_cache.yaml")
    ...
    def _GetFirefoxConfig(self, file_object, display_name):
        ...
        while file_object.get_offset() < to_read:
            ...
            try:
                ...
                # possible size.
                record_size = (
                    self._CACHE_ENTRY_HEADER_SIZE
                    + cache_entry.request_size
    ...
    def _ParseCacheEntry(self, parser_mediator, file_object, display_name, block_size):
        ...
        try:
            cache_entry_body = self._ReadStructureFromByteStream(
                cache_entry_body_data,
                file_offset,
                cache_entry_body_map,
                context=context,
            )
        except (ValueError, errors.ParseError) as exception:
            raise errors.ParseError(
    ...
    def _ValidateCacheEntryHeader(self, cache_entry_header):
        ...
        """
        return (
            cache_entry_header.request_size > 0
            and cache_entry_header.request_size < self._MAXIMUM_URL_LENGTH
            and cache_entry_header.major_format_version == 1
            and cache_entry_header.last_fetched_time > 0
            and cache_entry_header.fetch_count > 0
        )
    ...
    def ParseFileObject(self, parser_mediator, file_object):
        ...
        if not self._CACHE_FILENAME_REGEX.match(filename) and not filename.startswith(
            "_CACHE_00"
        ):
...
class FirefoxCache2Parser(BaseFirefoxCacheParser, dtfabric_helper.DtFabricHelper):
    ...
    def _ValidateCacheFileMetadataHeader(self, cache_file_metadata_header):
        ...
        """
        return (
            cache_file_metadata_header.key_size > 0
            and cache_file_metadata_header.key_size < self._MAXIMUM_URL_LENGTH
            and cache_file_metadata_header.format_version in (1, 2, 3)
            and cache_file_metadata_header.last_fetched_time > 0
            and cache_file_metadata_header.fetch_count > 0
        )
