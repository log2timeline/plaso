...
class BaseSpotlightFile(dtfabric_helper.DtFabricHelper):
    ...
    _DEFINITION_FILE = os.path.join(os.path.dirname(__file__), "spotlight_storedb.yaml")
...
class SpotlightStoreDatabaseParser(
    ...
    def _GetDateTimeMetadataItemValues(self, metadata_item, name):
        ...
        return [
            dfdatetime_cocoa_time.CocoaTime(timestamp=timestamp) for timestamp in value
        ]
    ...
    def _ParseMetadataItem(self, parser_mediator, metadata_item):
        ...
        used_times = (
            self._GetDateTimeMetadataItemValues(metadata_item, "kMDItemUsedDates") or []
        )
        date_time = self._GetDateTimeMetadataItemValue(
    ...
    def _ParseRecordPageValues(self, parser_mediator, page_data):
        ...
        while page_data_offset < page_data_size:
            bytes_read = self._ParseRecord(parser_mediator, page_data, page_data_offset)
    ...
    def _ReadRecordPage(self, file_object, file_offset):
        ...
        if page_header.property_table_type not in (0x00000009, 0x00001009, 0x00005009):
            raise errors.ParseError(
                f"Unsupported property table type: "
                f"0x{page_header.property_table_type:08x}"
            )
