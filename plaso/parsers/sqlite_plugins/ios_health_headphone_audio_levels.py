...
class IOSHealthHeadphoneAudioPlugin(interface.SQLitePlugin):
    ...
    QUERIES = [
        (
            (
                "SELECT samples.start_date AS start_date, samples.end_date AS "
                "end_date, strftime('%H:%M:%S', (samples.end_date - "
                "samples.start_date), 'unixepoch') AS total_time_duration, "
                "quantity_samples.quantity AS decibels, metadata_values.string_value "
                "AS bundle_name, source_devices.name AS device_name, "
                ...
                "ON metadata_values.object_id = samples.data_id LEFT JOIN "
                "metadata_keys ON metadata_keys.ROWID = metadata_values.key_id LEFT "
                "JOIN objects ON objects.data_id = samples.data_id LEFT JOIN "
                "data_provenances ON data_provenances.ROWID = objects.provenance LEFT "
                "JOIN source_devices ON source_devices.ROWID = "
                "data_provenances.device_id WHERE samples.data_type = 173 AND "
                "(metadata_keys.key IS NULL OR metadata_keys.key != "
    ...
    SCHEMAS = {
        ...
        "objects": ("CREATE TABLE objects (data_id INTEGER, provenance INTEGER)"),
        "data_provenances": (
