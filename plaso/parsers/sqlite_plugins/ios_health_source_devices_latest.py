...
class IOSHealthSourceDevicesLatestEventData(events.EventData):
  ...
  Attributes:
    ...
    sync_identity (str): sync identity information.
  ...
  def __init__(self):
    ...
    super(IOSHealthSourceDevicesLatestEventData, self).__init__(
...
class IOSHealthSourceDevicesLatestPlugin(interface.SQLitePlugin):
  """SQLite parser plugin for iOS Health Source Devices Latest data."""
  ...
  REQUIRED_STRUCTURE = {
      'source_devices': frozenset([
          ...
          'localIdentifier', 'sync_provenance', 'sync_identity',
          'creation_date'])}
  ...
  SCHEMAS = {
      'source_devices': (
          ...
          'sync_identity TEXT, creation_date INTEGER)')}
  ...
  def ParseSourceDevicesRow(
    ...
    event_data.firmware = self._GetRowValue(query_hash, row, 'firmware')
    ...
    event_data.sync_identity = self._GetRowValue(
        query_hash, row, 'sync_identity')

    parser_mediator.ProduceEventData(event_data)


sqlite.SQLiteParser.RegisterPlugin(IOSHealthSourceDevicesLatestPlugin)
