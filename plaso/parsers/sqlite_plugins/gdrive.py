...
class GoogleDrivePlugin(interface.SQLitePlugin):
    ...
    REQUIRED_STRUCTURE = {
        ...
        "cloud_relations": frozenset(["parent_resource_id", "child_resource_id"]),
        "local_entry": frozenset(
            ["inode_number", "filename", "modified", "checksum", "size"]
        ),
        "local_relations": frozenset(["child_inode_number", "parent_inode_number"]),
    }
    ...
    def ParseCloudEntryRow(
        ...
        event_data.creation_time = self._GetDateTimeRowValue(query_hash, row, "created")
