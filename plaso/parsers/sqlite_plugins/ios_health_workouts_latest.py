...
class IOSHealthWorkoutsLatestPlugin(interface.SQLitePlugin):
    ...
    QUERIES = [
        (
            (
                ...
                "goal_value, MAX(CASE WHEN ws.data_type = 10 THEN "
                "ROUND(ws.quantity, 2) END) AS total_active_kcal, MAX(CASE WHEN "
                "ws.data_type = 9 THEN ROUND(ws.quantity, 2) END) AS "
                "total_resting_kcal, MAX(CASE WHEN mk.key='HKAverageMETs' THEN "
                "ROUND(mv.numerical_value,1) END) AS avg_mets, MAX(CASE WHEN "
                "mk.key='_HKPrivateWorkoutMinHeartRate' THEN "
                "CAST(ROUND(mv.numerical_value*60) AS INT) END) AS min_hr_bpm, "
                "MAX(CASE WHEN mk.key='_HKPrivateWorkoutMaxHeartRate' THEN "
                "CAST(ROUND(mv.numerical_value*60) AS INT) END) AS max_hr_bpm, "
                "MAX(CASE WHEN mk.key='_HKPrivateWorkoutAverageHeartRate' THEN "
                "CAST(ROUND(mv.numerical_value*60) AS INT) END) AS avg_hr_bpm, "
                "MAX(CASE WHEN mk.key='HKWeatherTemperature' THEN "
                "ROUND(mv.numerical_value,2) END) AS temp_f, MAX(CASE WHEN "
                "mk.key='HKWeatherHumidity' THEN CAST(mv.numerical_value AS INT) END) "
                "AS humidity_pct, MAX(CASE WHEN "
                ...
                "ROUND(mv.numerical_value,2) END) AS max_elev_m, "
                "source_devices.hardware AS hardware, sources.name AS source_name, "
                "dp.source_version AS software_version, dp.tz_name AS timezone_name, "
                "o.creation_date AS added_cocoa FROM workout_activities AS wa LEFT "
                "JOIN workouts AS w ON w.data_id = wa.owner_id LEFT JOIN "
                "workout_statistics AS ws ON ws.workout_activity_id = wa.ROWID LEFT "
                "JOIN samples AS s ON s.data_id = w.data_id LEFT JOIN metadata_values "
                "AS mv ON mv.object_id = wa.owner_id LEFT JOIN metadata_keys AS mk ON "
                "mk.ROWID = mv.key_id LEFT JOIN objects AS o ON o.data_id = "
    ...
    SCHEMAS = {
        ...
        "source_devices": (
            "CREATE TABLE source_devices (ROWID INTEGER PRIMARY KEY, hardware " "TEXT)"
        ),
        "sources": ("CREATE TABLE sources (ROWID INTEGER PRIMARY KEY, name TEXT)"),
    }
