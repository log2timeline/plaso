...
class IMessagePlugin(interface.SQLitePlugin):
    ...
    REQUIRED_STRUCTURE = {
        "_SqliteDatabaseProperties": frozenset(["key", "value"]),
        "message": frozenset(
            ["date", "ROWID", "is_read", "is_from_me", "service", "text", "handle_id"]
        ),
        "handle": frozenset(["id", "ROWID"]),
        "attachment": frozenset(["filename", "ROWID"]),
        "message_attachment_join": frozenset(["message_id", "attachment_id"]),
    }
    ...
    QUERIES = [
        (
            ...
            "ON maj.attachment_id = attachment.ROWID",
            "ParseMessageRow",
        )
    ]
    ...
    def _GetClientVersion(self, cache, database):
        ...
        if not cache_results:
            ...
            cache.CacheQueryResults(query_result, "client_version", "key", ("value",))
            cache_results = cache.GetResults("client_version")
    ...
    def _GetDateTimeRowValue(self, query_hash, row, value_name):
        ...
        return dfdatetime_cocoa_time.CocoaTime(precision=precision, timestamp=timestamp)
