...
class IOSHealthWatchBySleepPeriodLatestPlugin(interface.SQLitePlugin):
    ...
    DATA_FORMAT = (
        "iOS Health Watch By Sleep Period Data from healthdb_secure.sqlite (iOS 17+)"
    )
    ...
    QUERIES = [
        (
            (
                ...
                "THEN 'CORE' WHEN cs.value = 4 THEN 'DEEP' WHEN cs.value = 5 THEN "
                "'REM' END AS sleep_value FROM samples s LEFT JOIN category_samples "
                "cs ON s.data_id = cs.data_id LEFT JOIN objects o ON s.data_id = "
                "o.data_id LEFT JOIN data_provenances dp ON o.provenance = dp.rowid "
                "WHERE s.data_type = 63 AND cs.value NOT IN (0, 1) AND "
                "dp.origin_product_type LIKE '%Watch%'), grouped_samples AS (SELECT *, "
                "CASE WHEN data_id - prev_data_id > 1 OR prev_data_id IS NULL THEN 1 "
                "ELSE 0 END AS is_new_group, SUM(CASE WHEN data_id - prev_data_id > 1 "
                "OR prev_data_id IS NULL THEN 1 ELSE 0 END) OVER (ORDER BY data_id) AS "
                "group_number FROM lagged_samples) SELECT MIN(start_date) AS "
                "start_date, MAX(end_date) AS end_date, SUM(CASE WHEN sleep_value IN "
                "('AWAKE', 'REM', 'CORE', 'DEEP') THEN (end_date - start_date) / 60.0 "
                "ELSE 0 END) AS total_duration, SUM(CASE WHEN sleep_value = 'AWAKE' "
                "THEN (end_date - start_date) / 60.0 ELSE 0 END) AS awake_duration, "
                "SUM(CASE WHEN sleep_value = 'REM' THEN (end_date - start_date) / 60.0 "
                "ELSE 0 END) AS rem_duration, SUM(CASE WHEN sleep_value = 'CORE' THEN "
    ...
    def ParseSleepRow(self, parser_mediator, query, row, **unused_kwargs):
        ...
        event_data.awake_duration_hms = self._SecondsToHMS(event_data.awake_duration)
        ...
        event_data.rem_percent = self._CalculatePercent(event_data.rem_duration, total)
