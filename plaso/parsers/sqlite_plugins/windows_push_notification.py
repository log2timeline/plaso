...
class WindowsPushNotificationPlugin(interface.SQLitePlugin):
    ...
    def ParseNotificationHandlerRow(self, parser_mediator, query, row, **unused_kwargs):
    ...
    def ParseNotificationRow(self, parser_mediator, query, row, **unused_kwargs):
        ...
        event_data.boot_time = self._GeFiletimeRowValue(query_hash, row, "BootId")
        event_data.expiration_time = self._GeFiletimeRowValue(
            query_hash, row, "ExpiryTime"
        )
        event_data.handler_identifier = self._GetRowValue(query_hash, row, "PrimaryId")
