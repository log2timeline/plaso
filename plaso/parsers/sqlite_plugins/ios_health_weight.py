...
class IOSHealthWeightPlugin(interface.SQLitePlugin):
    ...
    DATA_FORMAT = "iOS Health Weight SQLite database file healthdb_secure.sqlite"
    ...
    def ParseWeightRow(self, parser_mediator, query, row, **unused_kwargs):
        ...
        event_data.weight_value_ts_str = self._CopyToRfc3339String(event_data.date_time)
        event_data.weight_kg_str = self._GetRowValue(query_hash, row, "weight_kg_str")
        event_data.weight_lbs_str = self._GetRowValue(query_hash, row, "weight_lbs_str")
