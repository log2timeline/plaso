...
class IOSHealthWatchBySleepPeriodPlugin(interface.SQLitePlugin):
    ...
    QUERIES = [
        (
            (
                ...
                "SELECT *, CASE WHEN prev_data_id IS NULL OR data_id - "
                "prev_data_id > 1 THEN 1 ELSE 0 END AS is_new_group, SUM(CASE WHEN "
                "prev_data_id IS NULL OR data_id - prev_data_id > 1 THEN 1 ELSE 0 END) "
                "OVER (ORDER BY data_id) AS group_number FROM lagged_samples) SELECT "
                "MIN(start_date) AS start_date, MAX(end_date) AS end_date, "
                "MAX(data_type) AS data_type_id, MAX(origin_product_type) AS "
                "device_name, SUM(CASE WHEN sleep_value IN ('IN BED', 'ASLEEP') THEN "
                "duration_minutes ELSE 0 END) AS total_duration, SUM(CASE WHEN "
                "sleep_value = 'IN BED' THEN duration_minutes ELSE 0 END) AS "
                "in_bed_duration, CASE WHEN SUM(duration_minutes) > 0 THEN "
                "ROUND(SUM(CASE WHEN sleep_value = 'IN BED' THEN duration_minutes "
    ...
    def ParseSleepRowInBed(self, parser_mediator, query, row, **unused_kwargs):
        ...
        event_data.in_bed_duration = self._GetRowValue(
            query_hash, row, "in_bed_duration"
        )
        event_data.in_bed_percent = self._GetRowValue(query_hash, row, "in_bed_percent")
        event_data.start_time = self._GetDateTimeRowValue(query_hash, row, "start_date")
        event_data.total_duration = self._GetRowValue(query_hash, row, "total_duration")
