...
class IOSHealthWatchWornPlugin(interface.SQLitePlugin):
    ...
    REQUIRED_STRUCTURE = {"samples": frozenset(["start_date", "end_date", "data_type"])}
    ...
    QUERIES = [
        (
            (
                ...
                "PeriodData AS (SELECT *, (start_cocoa - prev_end_cocoa) AS "
                "gap_seconds, CASE WHEN (start_cocoa - prev_end_cocoa) > 3600 THEN 1 "
                "ELSE 0 END AS new_period FROM TimeData), PeriodGroup AS (SELECT *, "
                "SUM(new_period) OVER (ORDER BY start_cocoa ROWS BETWEEN UNBOUNDED "
                "PRECEDING AND CURRENT ROW) AS period_id FROM PeriodData), Summary AS "
                "(SELECT period_id, MIN(start_cocoa) AS period_start_cocoa, "
                "MAX(end_cocoa) AS period_end_cocoa, CAST((MAX(end_cocoa) - "
                "MIN(start_cocoa)) / 3600 AS INT) AS hours_worn FROM PeriodGroup "
                "GROUP BY period_id) SELECT s1.period_start_cocoa AS start_cocoa, "
                "s1.hours_worn AS hours_worn, s1.period_end_cocoa AS end_cocoa, "
                "CAST((s2.period_start_cocoa - s1.period_end_cocoa) / 3600 AS INT) "
                "AS hours_off_before_next FROM Summary s1 LEFT JOIN Summary s2 ON "
                "s1.period_id + 1 = s2.period_id ORDER BY s1.period_id"
            ),
            "ParseWatchWornRow",
        )
    ]
