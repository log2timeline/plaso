...
class ZeitgeistActivityDatabasePlugin(interface.SQLitePlugin):
    ...
    REQUIRED_STRUCTURE = {
        "actor": frozenset([]),
        "event": frozenset(["id", "subj_id", "timestamp"]),
        "uri": frozenset(["id"]),
    }

    QUERIES = [
        ("SELECT id, timestamp, subj_uri FROM event_view", "ParseZeitgeistEventRow")
    ]
    ...
    SCHEMAS = [
        {
            ...
            "payload": ("CREATE TABLE payload (id INTEGER PRIMARY KEY, value BLOB)"),
            "schema_version": (
