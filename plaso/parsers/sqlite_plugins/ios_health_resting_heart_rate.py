...
class IOSHealthRestingHeartRatePlugin(interface.SQLitePlugin):
    ...
    DATA_FORMAT = (
        "iOS Health Resting Heart Rate SQLite database file healthdb_secure.sqlite"
    )
    ...
    QUERIES = [
        (
            (
                ...
                "end_date, CAST(ROUND(quantity_samples.quantity) AS INTEGER) AS "
    ...
    def ParseRestingHeartRateRow(self, parser_mediator, query, row, **unused_kwargs):
