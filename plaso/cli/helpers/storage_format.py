...
class StorageFormatArgumentsHelper(interface.ArgumentsHelper):
    ...
    def AddArguments(cls, argument_group):
        ...
        storage_formats_string = ", ".join(session_storage_formats)
        argument_group.add_argument(
            "--storage_format",
            "--storage-format",
            action="store",
            choices=session_storage_formats,
            dest="storage_format",
            type=str,
            metavar="FORMAT",
            default=definitions.DEFAULT_STORAGE_FORMAT,
            help=(
                f"Format of the storage file, the default is: "
                f"{definitions.DEFAULT_STORAGE_FORMAT:s}. Supported options: "
                f"{storage_formats_string:s}"
            ),
        )

        storage_formats_string = ", ".join(task_storage_formats)
        argument_group.add_argument(
            "--task_storage_format",
            "--task-storage-format",
            action="store",
            choices=task_storage_formats,
            dest="task_storage_format",
            type=str,
            metavar="FORMAT",
            default=definitions.DEFAULT_STORAGE_FORMAT,
            help=(
                f"Format for task storage, the default is: "
                f"{definitions.DEFAULT_STORAGE_FORMAT:s}. Supported options: "
                f"{storage_formats_string:s}"
            ),
        )
