...
class PsortTool(analysis_tool.AnalysisTool, tool_options.OutputModuleOptions):
    """Psort CLI tool.
    ...
    def _CheckStorageFile(self, storage_file_path, check_readable_only=False):
        ...
        if not os.path.exists(storage_file_path):
            raise errors.BadConfigOption(f"No such storage file: {storage_file_path:s}")
        ...
        try:
            storage_file.Open(path=storage_file_path, read_only=check_readable_only)
    ...
    def _ParseProcessingOptions(self, options):
        ...
        Raises:
          BadConfigOption: if the options are invalid.
        """
        argument_helper_names = ["process_resources", "temporary_directory", "zeromq"]
        helpers_manager.ArgumentHelperManager.ParseOptions(
