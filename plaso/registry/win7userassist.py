...
class Win7UserAssistPlugin(win_registry_interface.KeyPlugin):
  """A registry plugin that parses Windows 7 UserAssist entries."""

  URL = [u'http://intotheboxes.files.wordpress.com/'
         '2010/04/intotheboxes_2010_q1.pdf']

  USERASSIST_STRUCT = construct.Struct(
    'userassist_entry', construct.Padding(4),
    construct.ULInt32('count'), construct.ULInt32('app_focus_count'),
    construct.ULInt32('focus_duration'), construct.Padding(44),
    construct.ULInt64('timestamp'))

  # The following definition for values can be found on MSDN.
  ...
  DESCRIPTION = 'Win7UserAssistPlugin'
  ...
  def GetEntries(self):
    ...
    for value in self._key.GetValues():
      ...
      text_dict = {}
      text_dict[name] = (
        u'[userassist_entry: {0}, '
        'Count: {1}, app_focus_count: {2}, '
