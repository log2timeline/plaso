...
class DataFormatInformationExtractor:
    ...
    def _GetDataFormatInformationFromPackage(self, package):
        ...
        for _, name, is_package in pkgutil.iter_modules(path=[package_path]):
            ...
            if is_package:
                sub_data_format_descriptors = self._GetDataFormatInformationFromPackage(
                    sub_package
                )
                ...
                for _, cls in inspect.getmembers(module_object, inspect.isclass):
                    if issubclass(
                        ...
                        if (
                            not parser_name
                            or parser_name in self._PARSER_NAME_IGNORE_LIST
                        ):
                            continue
                        ...
                        dtfabric_file = os.path.join(
                            package_path, "".join([name, ".yaml"])
                        )
