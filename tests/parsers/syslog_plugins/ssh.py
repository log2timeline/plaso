...
class SSHSyslogParserTest(test_lib.SyslogPluginTestCase):
  ...
  def testParse(self):
    ...
    self.assertEqual(syslog.SyslogLineEvent.DATA_TYPE, event.DATA_TYPE)

    event = event_objects[1]
    self.assertEqual(ssh.SSHLoginEvent.DATA_TYPE, event.DATA_TYPE)
    ...
    self.assertEqual(ssh.SSHFailedConnectionEvent.DATA_TYPE, event.DATA_TYPE)
    ...
    self.assertEqual(ssh.SSHOpenedConnectionEvent.DATA_TYPE, event.DATA_TYPE)
