...
class OneDriveTest(test_lib.ParserTestCase):
    ...
    def testRemovePKCS7Padding(self):
        ...
        unpadded_data = parser._RemovePKCS7Padding(b"\x08\x08\x08\x08\x08\x08\x08\x08")
        self.assertEqual(unpadded_data, b"")

        unpadded_data = parser._RemovePKCS7Padding(b"\x01\x02\x03\xff\x04\x04\x04\x04")
        self.assertEqual(unpadded_data, b"\x01\x02\x03\xff")

        unpadded_data = parser._RemovePKCS7Padding(b"\x01\x02\x03\x04\x05\x06\xff\x01")
    ...
    def testParse(self):
        ...
        expected_event_values = {
            ...
            "code_function_name": "SyncServiceProxy::OnDownloadedSelectiveSyncEntries",
            "decoded_parameters": [
                "test document.docx",
                "Document",
                "B201A0178F192B2B!107",
                "B201A0178F192B2B!107.10",
                "B201A0178F192B2B!101",
            ],
            "raw_parameters": (
