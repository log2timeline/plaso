...
class TestMRUListExStringWindowsRegistryPlugin(test_lib.RegistryPluginTestCase):
    ...
    def _CreateTestKey(self):
        ...
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "1", data=value_data, data_type=dfwinreg_definitions.REG_BINARY, offset=612
        )
        registry_key.AddValue(registry_value)

        value_data = "C:\\looks_legit.exe".encode("utf_16_le")
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "2", data=value_data, data_type=dfwinreg_definitions.REG_SZ, offset=1001
        )
        registry_key.AddValue(registry_value)
...
class TestMRUListExShellItemListWindowsRegistryPlugin(test_lib.RegistryPluginTestCase):
