...
class NetworksWindowsRegistryPluginTest(test_lib.RegistryPluginTestCase):
    ...
    def _CreateTestKey(self):
        ...
        value_data = b"\xde\x07\x0c\x00\x02\x00\x10\x00\x08\x00\x04\x00\x27\x00\x6a\x00"
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "DateCreated", data=value_data, data_type=dfwinreg_definitions.REG_BINARY
        )
        profile1_key.AddValue(registry_value)

        value_data = b"\xdf\x07\x01\x00\x02\x00\x1b\x00\x0f\x00\x0f\x00\x1b\x00\xc5\x03"
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "DateLastConnected",
            data=value_data,
        ...
        value_data = "My Awesome Wifi Hotspot".encode("utf_16_le")
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "ProfileName", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        profile1_key.AddValue(registry_value)
        ...
        value_data = b"\xde\x07\x05\x00\x02\x00\x06\x00\x11\x00\x02\x00\x13\x00\x1b\x03"
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "DateCreated", data=value_data, data_type=dfwinreg_definitions.REG_BINARY
        )
        profile2_key.AddValue(registry_value)

        value_data = b"\xde\x07\x05\x00\x02\x00\x06\x00\x11\x00\x07\x00\x36\x00\x0a\x00"
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "DateLastConnected",
            data=value_data,
            data_type=dfwinreg_definitions.REG_BINARY,
        ...
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "DnsSuffix", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        unmanaged_subkey.AddValue(registry_value)

        value_data = "Network".encode("utf_16_le")
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "FirstNetwork", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        unmanaged_subkey.AddValue(registry_value)

        value_data = "{C1C57B58-BFE2-428B-818C-9D69A873AD3D}".encode("utf_16_le")
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "ProfileGuid", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        unmanaged_subkey.AddValue(registry_value)
