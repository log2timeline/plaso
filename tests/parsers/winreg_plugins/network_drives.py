...
class NetworkDrivesPluginTest(test_lib.RegistryPluginTestCase):
    ...
    def _CreateTestKey(self):
        ...
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "ProviderName", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        h_key.AddValue(registry_value)
        ...
        value_data = "\\\\acme.local\\Shares\\User_Data\\John.Doe".encode("utf_16_le")
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "RemotePath", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        h_key.AddValue(registry_value)
