...
class IOSHealthRestingHeartRatePluginGoldenTest(test_lib.SQLitePluginTestCase):
    ...
    def testProcess(self):
        ...
        if not (
            getattr(event_data, "resting_heart_rate", None) == 75
            and getattr(event_data, "hardware", None) == "Watch4,3"
            and getattr(event_data, "source", None) == "This Is's Apple Watch"
        ):
            matched = None
            for i in range(number_of_event_data):
                ed = storage_writer.GetAttributeContainerByIndex("event_data", i)
                if (
                    getattr(ed, "resting_heart_rate", None) == 75
                    and getattr(ed, "hardware", None) == "Watch4,3"
                    and getattr(ed, "source", None) == "This Is's Apple Watch"
                ):
                    matched = ed
                    break
