...
class CLIHelperImportTest(test_lib.BaseTestCase):
  ...
  def testParsersImported(self):
    ...
    self._AssertFilesImportedInInit(self._CLI_HELPERS_PATH, self._IGNORABLE_FILES)

  def testPluginsImported(self):
    """Tests that all plugins are imported."""
    parsers_glob = '{0:s}/*_plugins/'.format(self._CLI_HELPERS_PATH)
    plugin_directories = glob.glob(parsers_glob)
    for plugin_directory in plugin_directories:
      plugin_directory_path = os.path.join(
          self._CLI_HELPERS_PATH, plugin_directory)
