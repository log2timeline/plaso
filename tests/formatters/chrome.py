...
class ChromeHistoryTypedCountFormatterHelperTest(test_lib.EventFormatterTestCase):
    ...
    def testFormatEventValues(self):
        ...
        formatter_helper.FormatEventValues(output_mediator, event_values)
        self.assertEqual(event_values["url_typed_string"], "(URL typed 1 time)")

        event_values = {"typed_count": 3}
        formatter_helper.FormatEventValues(output_mediator, event_values)
        self.assertEqual(event_values["url_typed_string"], "(URL typed 3 times)")
