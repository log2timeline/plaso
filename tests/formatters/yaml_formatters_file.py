...
class YAMLFormattersFileTest(shared_test_lib.BaseTestCase):
    ...
    _FORMATTERS_YAML = {
        ...
        "message": ["{display_name}", "Type: {file_entry_type}", "({unallocated})"],
    ...
    def testReadFormatterDefinition(self):
        ...
        formatter = test_formatters_file._ReadFormatterDefinition(self._FORMATTERS_YAML)
        ...
        with self.assertRaises(errors.ParseError):
            test_formatters_file._ReadFormatterDefinition(
                {"type": "conditional", "data_type": "test:fs:stat"}
            )
