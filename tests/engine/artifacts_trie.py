...
class ArtifactsTrieTest(unittest.TestCase):
    ...
    def test_add_path(self):
        ...
        self.assertIn("file.txt", to_children)
        self.assertIn("another.txt", to_children)
        self.assertEqual(
            to_children["file.txt"].artifacts_names,
            ["artifact1"],
        )
        self.assertEqual(to_children["another.txt"].artifacts_names, ["artifact2"])
    ...
    def test_add_path_with_glob(self):
        ...
        self.assertIn("to", path_children)
        to_children = path_children["to"].children
        self.assertIn("*.txt", to_children)
        self.assertIn("dir**", to_children)
        self.assertEqual(to_children["*.txt"].artifacts_names, ["artifact1"])
        self.assertEqual(to_children["dir**"].artifacts_names, ["artifact2"])
    ...
    def test_get_matching_artifacts_with_multiple_globs(self):
        ...
        matches = self.trie.GetMatchingArtifacts("/path/to/dir/subdir/file.txt", "/")
        self.assertIn("artifact1", matches)
        self.assertNotIn("artifact2", matches)

        matches = self.trie.GetMatchingArtifacts("/path/to/dir/subdir/another.txt", "/")
        self.assertIn("artifact2", matches)
        self.assertNotIn("artifact1", matches)
    ...
    def test_get_matching_artifacts_single_asterisk(self):
        ...
        matches = self.trie.GetMatchingArtifacts("/path/dir/data/test.txt", "/")
        self.assertNotIn("artifact1", matches)
        self.assertIn("artifact2", matches)
    ...
    def test_get_matching_artifacts_windows_paths(self):
        ...
        matches = self.trie.GetMatchingArtifacts("\\Users\\test\\AppData\\Local", "\\")
        self.assertIn("artifact2", matches)
        self.assertNotIn("artifact1", matches)
    ...
    def test_add_path_with_mixed_separators(self):
        ...
        self.trie.AddPath("artifact2", "\\another\\mixed path\\style to-match_it", "\\")
    ...
    def test_get_matching_artifacts_sanitized_paths(self):
        ...
        matches = self.trie.GetMatchingArtifacts("/path/to/file__.txt", "/")
        self.assertIn("artifact1", matches)

        # Test with a different path that sanitizes to the same value.
        matches = self.trie.GetMatchingArtifacts("/another/path/fo_r_/file.txt", "/")
        self.assertIn("artifact2", matches)
