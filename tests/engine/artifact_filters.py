...
class ArtifactDefinitionsFiltersHelperTest(shared_test_lib.BaseTestCase):
    ...
    def testBuildFindSpecsFromFileSourcePath(self):
        ...
        self.assertEqual(path_trie_node.artifacts_names, [])
        ...
        self.assertEqual(path_trie_node_children["Windows"].artifacts_names, [])
        self.assertIn("test_data", path_trie_node_children["Windows"].children)

        test_data_children = path_trie_node_children["test_data"].children
        self.assertEqual(path_trie_node_children["test_data"].artifacts_names, [])
        self.assertEqual(len(test_data_children), 1)
        self.assertIn("*", test_data_children)

        star_children = test_data_children["*"].children
        self.assertEqual(test_data_children["*"].artifacts_names, [artifact_name])
        self.assertEqual(len(star_children), 1)
        self.assertIn("*", star_children)

        self.assertEqual(star_children["*"].artifacts_names, [artifact_name])
        self.assertEqual(len(star_children["*"].children), 1)
        self.assertIn("*", star_children["*"].children)

        self.assertEqual(path_trie_node_children["home"].artifacts_names, [])
        self.assertIn("testuser2", path_trie_node_children["home"].children)

        self.assertEqual(path_trie_node_children["Users"].artifacts_names, [])
        self.assertIn("testuser2", path_trie_node_children["Users"].children)

        self.assertEqual(path_trie_node_children["homes"].artifacts_names, [])
    ...
    def testBuildFindSpecsFromRegistrySourceKey(self):
        ...
        key_path = "HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\**"
        find_specs = test_filter_file._BuildFindSpecsFromRegistrySourceKey(key_path)
        ...
        self.assertEqual(find_specs[0]._key_path_segments, expected_key_path_segments)
        ...
        expected_key_path_segments = ["HKEY_LOCAL_MACHINE", "Software", ".*", "Classes"]
