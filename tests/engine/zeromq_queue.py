...
class testZeroMQPushPullQueues(test_lib.EngineTestCase):
  ...
  def testSocketCreation(self):
    ...
    push_queue = zeromq_queue.ZeroMQPushQueue(delay_start=False)
    pull_queue = zeromq_queue.ZeroMQPullQueue(delay_start=False)
  ...
  def testQueueStart(self):
    ...
    push_queue = zeromq_queue.ZeroMQPushQueue(delay_start=True)
    pull_queue = zeromq_queue.ZeroMQPullQueue(delay_start=True)
  ...
  def testItemCanBeQueuedAndDequeued(self):
    ...
    push_queue = zeromq_queue.ZeroMQPushQueue(delay_start=False)
    pull_queue = zeromq_queue.ZeroMQPullQueue(
        connect=True, delay_start=False, port=push_queue.port)
