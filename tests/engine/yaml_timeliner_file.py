...
class YAMLTimelinerConfigurationFileTest(shared_test_lib.BaseTestCase):
    ...
    _FORMATTERS_YAML = {
        ...
        "attribute_mappings": [
            {"name": "access_time", "description": "Last Access Time"}
        ],
    ...
    def testReadFromFileObject(self):
        ...
        with io.open(test_file_path, "r", encoding="utf-8") as file_object:
            timeliner_definitions = list(
                test_timeliner_file._ReadFromFileObject(file_object)
            )
    ...
    def testReadFromFile(self):
        ...
        timeliner_definitions = list(test_timeliner_file.ReadFromFile(test_file_path))
