...
class DateTimeFileEntryFilterTest(shared_test_lib.BaseTestCase):
    ...
    def testMatches(self):
        ...
        test_filter = file_entry_filters.DateTimeFileEntryFilter()
        test_filter.AddDateTimeRange("bkup", start_time_string="2012-02-02 12:12:12")
        self.assertTrue(test_filter.Matches(file_entry))

        # Just end date set.
        test_filter = file_entry_filters.DateTimeFileEntryFilter()
        test_filter.AddDateTimeRange("mtime", end_time_string="2012-05-25 15:59:55")
        self.assertTrue(test_filter.Matches(file_entry))

        # Just with a start date but within range.
        test_filter = file_entry_filters.DateTimeFileEntryFilter()
        test_filter.AddDateTimeRange("atime", start_time_string="2012-03-25 15:59:55")
        self.assertTrue(test_filter.Matches(file_entry))

        # And now with a start date, but out of range.
        test_filter = file_entry_filters.DateTimeFileEntryFilter()
        test_filter.AddDateTimeRange("ctime", start_time_string="2012-05-25 15:59:55")
        self.assertFalse(test_filter.Matches(file_entry))
    ...
    def testPrint(self):
        ...
        test_filter.AddDateTimeRange("atime", end_time_string="2012-05-25 15:59:25")

        test_filter.AddDateTimeRange("mtime", start_time_string="2012-05-25 15:59:20")
...
class ExtensionsFileEntryFilterTest(shared_test_lib.BaseTestCase):
    ...
    def testPrint(self):
        ...
        expected_output = [b"\textensions: exe, pdf", b""]
...
class SignaturesFileEntryFilterTest(shared_test_lib.BaseTestCase):
    ...
    def testGetScanner(self):
        ...
        file_scanner = test_filter._GetScanner(specification_store, ["negative_offset"])
        self.assertIsNotNone(file_scanner)

        file_scanner = test_filter._GetScanner(specification_store, ["positive_offset"])
        self.assertIsNotNone(file_scanner)
...
class FileEntryFilterCollectionTest(shared_test_lib.BaseTestCase):
    ...
    def testGetMatchingArtifacts(self):
        ...
        test_filter_collection = file_entry_filters.FileEntryFilterCollection()
        trie = artifacts_trie.ArtifactsTrie()
        trie.AddPath("artifact1", f"{os.sep}path{os.sep}to{os.sep}file.txt", os.sep)
        trie.AddPath("artifact2", f"{os.sep}path{os.sep}to{os.sep}dir{os.sep}", os.sep)
        test_filter_collection.SetArtifactsTrie(trie)
